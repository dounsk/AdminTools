...
class activity(Pbi):
	...
	def getLogs(self,startDateTime,endDateTime,filter=None):
		url = "https://api.powerbi.cn/v1.0/myorg/admin/activityevents?startDateTime='%s'&endDateTime='%s'" % (startDateTime,endDateTime)
		if filter!= None:
			url  += "&$filter=%s" % filter
		...
		while ret['continuationToken'] != None:
			urlcon = "https://api.powerbi.cn/v1.0/myorg/admin/activityevents?continuationToken='%s'" % ret['continuationToken']

...
LastEditTime : 2023-03-22 10:20:36
FilePath     : \Scripts\Python\ScheduledTasks\tasks\Qs_task_performance.pyw
...
saved_image = "D:/OneDrive - 8088/Scripts/Python/Data/get/QlikSenseTaskSchedulerLoad_" + date  + ".png"
...
for node_name in node_list:
    ...
    # list 每个 node 最大排队数量
    task_Queued += "<li> The max queued task for {} is <b> {} </b> at <b> {} </b></li>".format(node_name, max_queued_number, max_time)
...
fig.savefig("{}".format(saved_image))
with open("{}".format(saved_image), 'rb') as f:
    img_data = f.read()
image = MIMEImage(img_data)
image.add_header('Content-ID', '<{}>'.format('Qstaskload'))
msg.attach(image)
...
html += '<img src="cid:{}">'.format('Qstaskload')

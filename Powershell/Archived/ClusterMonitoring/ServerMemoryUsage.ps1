...
If($RAM_Usage -ge 75 ){
    ...
    $tital = '❗ '+((((Get-WmiObject -Class win32_OperatingSystem).TotalVisibleMemorySize-(Get-WmiObject -Class win32_OperatingSystem).FreePhysicalMemory)/(Get-WmiObject -Class win32_OperatingSystem).TotalVisibleMemorySize)).ToString('#0.##%', $CultInfo) +" Memory Usage on" + (((ipconfig|select-string "IPv4"|out-string).Split(":")[1]) -split '\r?\n')[0] + " - $env:COMPUTERNAME"

...
$Export = "C:\Users\"+$env:UserName+"\Downloads\"+$env:COMPUTERNAME+"_QVDsList-"+$Directory+"_$(Get-Date -Format 'yyyy-MM-dd.HHMM').txt"
$msg1 ="Name , FullName , Size(MB) ,  CreationTime , LastAccessTime ,  LastWriteTime"
write-output $msg1 >> $Export
Get-ChildItem -Path $startFolder -Filter *.qvd -recurse | Where-Object {$_.PSIsContainer -eq $false}| ForEach-Object -Process{
